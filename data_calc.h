    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            if (cobjint == 1)
            {
                printf("Trying to calc/write file %s\n", filename);
                fflush(stdout);
                /* j selects the table type (IC emission or Gamma transition
                   rate); i selects the radiation field (CMB or FIR). */
                ICo.do_3D_IC[j][i] = do3D_IC( init_do_2D_IC_version[j], dndEphot3D[i], FIELD_num[i], FIELD_lim[i], 
