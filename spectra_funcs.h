...
double q_pi( double E_pi__GeV, double n_H__cmm3, double C_p, double T_p_cutoff__GeV, gsl_spline_object_1D gso1D_fcal )
{
    ...
    /*
     * The low-energy neutrino integral samples x up to unity, which can map
     * photon-grid energies far below the tabulated proton/calorimetry range.
     * Such protons cannot contribute pion emission; do not ask GSL to
     * interpolate outside the physical table domain.
     */
...
double tau_FF_MK( double E_gam__GeV, double Sigma_SFR__Msolyrm1pcm2, double T_e__K ) 
{
    ...
    double nu_gam__Hz = E_gam__GeV/h__GeVs;
    double g_ff;
    /* Paper equation (45) switches branches at nu_9 = 1 (1 GHz). */
    if ( nu_gam__Hz > 1e9 )
    {
        //Draine 10.2
...
//        g_ff = 4.691*(1.-0.118*log( Z*( nu_gam__Hz/1e10 )/pow(T_e__K/1e4, 3./2.)));
    ...
    /* In equations (44) and (51), f_e n_i^2 cancels between kappa_ff
       and the Stromgren-sphere covering fraction. alpha_nuff above is
       already the coefficient with that density factor omitted. */
