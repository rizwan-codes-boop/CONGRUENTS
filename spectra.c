...
{
    gsl_set_error_handler(congruents_gsl_error_handler);
    if (argc != 4)
    {
        fprintf(stderr, "Usage: %s GALAXY_CATALOGUE DATA_DIRECTORY OUTPUT_DIRECTORY\n", argv[0]);
        return 2;
    }
...
//  double spec_emit[n_Esteps];
    ...
    for (i = 0; i < n_gal; i++)
    {
        ...
        nphot_params[0] = T_0_CMB__K * (1.+z[i]);//CMB temperature in K
        nphot_params[1] = T_dust__K[i];//dust temperature in K
        nphot_params[2] = M_star__Msol[i];//stellar mass in Msol
        nphot_params[3] = SFR__Msolyrm1[i];//star formation rate in Msol/yr
        nphot_params[4] = Re__kpc[i];//effective radius in kpc
        nphot_params[5] = h__pc[i];//scale height in pc
...
//    return 0;
    ...
    double **tau_loss_z1_SY = malloc(sizeof *tau_loss_z1_SY * n_gal);//synchrotron loss times at z1
    if (tau_loss_z1_SY){for (i = 0; i < n_gal; i++){tau_loss_z1_SY[i] = malloc(sizeof *tau_loss_z1_SY[i] * n_T_CR);}}
    double **tau_loss_z1_BS = malloc(sizeof *tau_loss_z1_BS * n_gal);//bremsstrahlung loss times at z1
    if (tau_loss_z1_BS){for (i = 0; i < n_gal; i++){tau_loss_z1_BS[i] = malloc(sizeof *tau_loss_z1_BS[i] * n_T_CR);}}
    double **tau_loss_z1_IC = malloc(sizeof *tau_loss_z1_IC * n_gal);//inverse Compton loss times at z1
    if (tau_loss_z1_IC){for (i = 0; i < n_gal; i++){tau_loss_z1_IC[i] = malloc(sizeof *tau_loss_z1_IC[i] * n_T_CR);}}
    double **tau_loss_z1_IO = malloc(sizeof *tau_loss_z1_IO * n_gal);//ionization loss times at z1
    if (tau_loss_z1_IO){for (i = 0; i < n_gal; i++){tau_loss_z1_IO[i] = malloc(sizeof *tau_loss_z1_IO[i] * n_T_CR);}}
    double **tau_loss_z1_DI = malloc(sizeof *tau_loss_z1_DI * n_gal);//diffusive loss times at z1
    if (tau_loss_z1_DI){for (i = 0; i < n_gal; i++){tau_loss_z1_DI[i] = malloc(sizeof *tau_loss_z1_DI[i] * n_T_CR);}}

    double **tau_loss_z2_SY = malloc(sizeof *tau_loss_z2_SY * n_gal);//synchrotron loss times at z2
    if (tau_loss_z2_SY){for (i = 0; i < n_gal; i++){tau_loss_z2_SY[i] = malloc(sizeof *tau_loss_z2_SY[i] * n_T_CR);}}
    double **tau_loss_z2_BS = malloc(sizeof *tau_loss_z2_BS * n_gal);//bremsstrahlung loss times at z2
    if (tau_loss_z2_BS){for (i = 0; i < n_gal; i++){tau_loss_z2_BS[i] = malloc(sizeof *tau_loss_z2_BS[i] * n_T_CR);}}
    double **tau_loss_z2_IC = malloc(sizeof *tau_loss_z2_IC * n_gal);//inverse Compton loss times at z2
    if (tau_loss_z2_IC){for (i = 0; i < n_gal; i++){tau_loss_z2_IC[i] = malloc(sizeof *tau_loss_z2_IC[i] * n_T_CR);}}
    double **tau_loss_z2_IO = malloc(sizeof *tau_loss_z2_IO * n_gal);//ionization loss times at z2
    if (tau_loss_z2_IO){for (i = 0; i < n_gal; i++){tau_loss_z2_IO[i] = malloc(sizeof *tau_loss_z2_IO[i] * n_T_CR);}}
    double **tau_loss_z2_DI = malloc(sizeof *tau_loss_z2_DI * n_gal);//diffusive loss times at z2
    if (tau_loss_z2_DI){for (i = 0; i < n_gal; i++){tau_loss_z2_DI[i] = malloc(sizeof *tau_loss_z2_DI[i] * n_T_CR);}}

    double **tau_loss_protons_PP = malloc(sizeof *tau_loss_protons_PP * n_gal);//proton-proton loss times
    if (tau_loss_protons_PP){for (i = 0; i < n_gal; i++){tau_loss_protons_PP[i] = malloc(sizeof *tau_loss_protons_PP[i] * n_T_CR);}}
    double **tau_loss_protons_DI = malloc(sizeof *tau_loss_protons_DI * n_gal);//proton diffusive loss times
    if (tau_loss_protons_DI){for (i = 0; i < n_gal; i++){tau_loss_protons_DI[i] = malloc(sizeof *tau_loss_protons_DI[i] * n_T_CR);}}



    double **spec_pi = malloc(sizeof *spec_pi * n_gal);//pion decay spectra
    if (spec_pi){for (i = 0; i < n_gal; i++){spec_pi[i] = malloc(sizeof *spec_pi[i] * n_E_gam);}}

    double **spec_pi_fcal1 = malloc(sizeof *spec_pi_fcal1 * n_gal);//pion decay spectra with fcal=1
    if (spec_pi_fcal1){for (i = 0; i < n_gal; i++){spec_pi_fcal1[i] = malloc(sizeof *spec_pi_fcal1[i] * n_E_gam);}}

    double **spec_nu = malloc(sizeof *spec_nu * n_gal);//neutrino spectra
    if (spec_nu){for (i = 0; i < n_gal; i++){spec_nu[i] = malloc(sizeof *spec_nu[i] * n_E_gam);}}



    double **spec_IC_1_z1 = malloc(sizeof *spec_IC_1_z1 * n_gal);//IC spectra from primary electrons at z1
    if (spec_IC_1_z1){for (i = 0; i < n_gal; i++){spec_IC_1_z1[i] = malloc(sizeof *spec_IC_1_z1[i] * n_E_gam);}}
    double **spec_IC_2_z1 = malloc(sizeof *spec_IC_2_z1 * n_gal);//IC spectra from secondary electrons at z1
    if (spec_IC_2_z1){for (i = 0; i < n_gal; i++){spec_IC_2_z1[i] = malloc(sizeof *spec_IC_2_z1[i] * n_E_gam);}}
    double **spec_IC_1_z2 = malloc(sizeof *spec_IC_1_z2 * n_gal);//IC spectra from primary electrons at z2
    if (spec_IC_1_z2){for (i = 0; i < n_gal; i++){spec_IC_1_z2[i] = malloc(sizeof *spec_IC_1_z2[i] * n_E_gam);}}
    double **spec_IC_2_z2 = malloc(sizeof *spec_IC_2_z2 * n_gal);//IC spectra from secondary electrons at z2
    if (spec_IC_2_z2){for (i = 0; i < n_gal; i++){spec_IC_2_z2[i] = malloc(sizeof *spec_IC_2_z2[i] * n_E_gam);}}

    double **spec_BS_1_z1 = malloc(sizeof *spec_BS_1_z1 * n_gal);//BS spectra from primary electrons at z1
    if (spec_BS_1_z1){for (i = 0; i < n_gal; i++){spec_BS_1_z1[i] = malloc(sizeof *spec_BS_1_z1[i] * n_E_gam);}}
    double **spec_BS_2_z1 = malloc(sizeof *spec_BS_2_z1 * n_gal);//BS spectra from secondary electrons at z1
...
//    if (spec_BS_2_z2){for (i = 0; i < n_gal; i++){spec_BS_2_z2[i] = malloc(sizeof *spec_BS_2_z2[i] * n_E_gam);}}
    ...
    double **spec_SY_1_z1 = malloc(sizeof *spec_SY_1_z1 * n_gal);//synchrotron spectra from primary electrons at z1
    if (spec_SY_1_z1){for (i = 0; i < n_gal; i++){spec_SY_1_z1[i] = malloc(sizeof *spec_SY_1_z1[i] * n_E_gam);}}
    double **spec_SY_2_z1 = malloc(sizeof *spec_SY_2_z1 * n_gal);//synchrotron spectra from secondary electrons at z1
    if (spec_SY_2_z1){for (i = 0; i < n_gal; i++){spec_SY_2_z1[i] = malloc(sizeof *spec_SY_2_z1[i] * n_E_gam);}}
    double **spec_SY_1_z2 = malloc(sizeof *spec_SY_1_z2 * n_gal);//synchrotron spectra from primary electrons at z2
    if (spec_SY_1_z2){for (i = 0; i < n_gal; i++){spec_SY_1_z2[i] = malloc(sizeof *spec_SY_1_z2[i] * n_E_gam);}}
    double **spec_SY_2_z2 = malloc(sizeof *spec_SY_2_z2 * n_gal);//synchrotron spectra from secondary electrons at z2
    if (spec_SY_2_z2){for (i = 0; i < n_gal; i++){spec_SY_2_z2[i] = malloc(sizeof *spec_SY_2_z2[i] * n_E_gam);}}




    double **Q_e_1_z1 = malloc(sizeof *Q_e_1_z1 * n_gal);//electron injection spectra from primary electrons at z1
    if (Q_e_1_z1){for (i = 0; i < n_gal; i++){Q_e_1_z1[i] = malloc(sizeof *Q_e_1_z1[i] * n_T_CR);}}
    double **Q_e_1_z2 = malloc(sizeof *Q_e_1_z2 * n_gal);//electron injection spectra from primary electrons at z2
    if (Q_e_1_z2){for (i = 0; i < n_gal; i++){Q_e_1_z2[i] = malloc(sizeof *Q_e_1_z2[i] * n_T_CR);}}
    double **Q_e_2_z1 = malloc(sizeof *Q_e_2_z1 * n_gal);//electron injection spectra from secondary electrons at z1
    if (Q_e_2_z1){for (i = 0; i < n_gal; i++){Q_e_2_z1[i] = malloc(sizeof *Q_e_2_z1[i] * n_T_CR);}}
    double **Q_e_2_z2 = malloc(sizeof *Q_e_2_z2 * n_gal);//electron injection spectra from secondary electrons at z2
    if (Q_e_2_z2){for (i = 0; i < n_gal; i++){Q_e_2_z2[i] = malloc(sizeof *Q_e_2_z2[i] * n_T_CR);}}

    double **q_p_SS_z1 = malloc(sizeof *q_p_SS_z1 * n_gal);//proton steady state spectra at z1
    if (q_p_SS_z1){for (i = 0; i < n_gal; i++){q_p_SS_z1[i] = malloc(sizeof *q_p_SS_z1[i] * n_T_CR);}}
    double **q_e_SS_1_z1 = malloc(sizeof *q_e_SS_1_z1 * n_gal);//primary electron steady state spectra at z1
    if (q_e_SS_1_z1){for (i = 0; i < n_gal; i++){q_e_SS_1_z1[i] = malloc(sizeof *q_e_SS_1_z1[i] * n_T_CR);}}
    double **q_e_SS_2_z1 = malloc(sizeof *q_e_SS_2_z1 * n_gal);//secondary electron steady state spectra at z1
    if (q_e_SS_2_z1){for (i = 0; i < n_gal; i++){q_e_SS_2_z1[i] = malloc(sizeof *q_e_SS_2_z1[i] * n_T_CR);}}
    double **q_e_SS_1_z2 = malloc(sizeof *q_e_SS_1_z2 * n_gal);//primary electron steady state spectra at z2
    if (q_e_SS_1_z2){for (i = 0; i < n_gal; i++){q_e_SS_1_z2[i] = malloc(sizeof *q_e_SS_1_z2[i] * n_T_CR);}}
    double **q_e_SS_2_z2 = malloc(sizeof *q_e_SS_2_z2 * n_gal);//secondary electron steady state spectra at z2
    if (q_e_SS_2_z2){for (i = 0; i < n_gal; i++){q_e_SS_2_z2[i] = malloc(sizeof *q_e_SS_2_z2[i] * n_T_CR);}}


    double **tau_FF = malloc(sizeof *tau_FF * n_gal);//free-free optical depth array
    if (tau_FF){for (i = 0; i < n_gal; i++){tau_FF[i] = malloc(sizeof *tau_FF[i] * n_E_gam);}}


    double **spec_FF = malloc(sizeof *spec_FF * n_gal);//free-free spectra
    if (spec_FF){for (i = 0; i < n_gal; i++){spec_FF[i] = malloc(sizeof *spec_FF[i] * n_E_gam);}}



    double E_crit__GeV;//critical energy for synchrotron emission

    double **E_loss_leptons = malloc(sizeof *E_loss_leptons * n_gal);//lepton energy loss rates
    if (E_loss_leptons){for (i = 0; i < n_gal; i++){E_loss_leptons[i] = malloc(sizeof *E_loss_leptons[i] * 16);}}

    double **E_loss_nucrit = malloc(sizeof *E_loss_nucrit * n_gal);//lepton energy loss rates at critical synchrotron energy
    if (E_loss_nucrit){for (i = 0; i < n_gal; i++){E_loss_nucrit[i] = malloc(sizeof *E_loss_nucrit[i] * 10);}}

    double **Lradio = malloc(sizeof *Lradio * n_gal);//radio luminosity
    ...
    for (i = 0; i < n_gal; i++)
    {
        ...
        /*
         * Spline coefficients are read-only, but GSL accelerators are mutable
         * caches. The firstprivate copies otherwise share cache pointers
         * between OpenMP workers, which produces races and invalid lookups.
         */
        gso2D_BS.xacc = gsl_interp_accel_alloc();
        gso2D_BS.yacc = gsl_interp_accel_alloc();
        gso1D_SY.acc = gsl_interp_accel_alloc();
...
/* The paper's corrected-LMC experiment changes proton calorimetry only;
   it does not reduce the primary-electron injection normalization. */
        }
...
*/
        ...
        /* Only these private accelerator caches belong to this iteration. */
        gsl_interp_accel_free( gso2D_BS.xacc );
        gsl_interp_accel_free( gso2D_BS.yacc );
        gsl_interp_accel_free( gso1D_SY.acc );
...
//    free2D( n_gal, q_p_SS_z1 );
    ...
    for (k = 0; k < 14; k++)
    {
        for (i = 0; i < n_gal; i++)
        {
            ...
            for (j = 0; j < n_E_gam; j++)
            {
                /* Redshifting contracts the tabulated output-energy domain.
                 * There is no model spectrum above that domain, so the
                 * contribution is zero rather than a GSL extrapolation. */
                if (E_gam__GeV[j] < gso1D_data.x_lim[0]
                    || E_gam__GeV[j] > gso1D_data.x_lim[1])
                {
                    data[i][j] = 0.;
                }
                else
